<?php

declare(strict_types=1);

namespace App\Policies;

use App\Enums\Permission;
use App\Models\Interaction;
use App\Models\User;

final class InteractionPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return $user->can(Permission::ViewAnyInteraction);
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, Interaction $interaction): bool
    {
        return $user->can(Permission::ViewInteraction);
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return $user->can(Permission::CreateInteraction);
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, Interaction $interaction): bool
    {
        return $user->can(Permission::UpdateInteraction);
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, Interaction $interaction): bool
    {
        return $user->can(Permission::DeleteInteraction);
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, Interaction $interaction): bool
    {
        return $user->can(Permission::RestoreInteraction);
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, Interaction $interaction): bool
    {
        return $user->can(Permission::ForceDeleteInteraction);
    }
}
