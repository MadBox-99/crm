<div class="flex h-[calc(100vh-12rem)] flex-col bg-white dark:bg-gray-900 rounded-xl shadow-lg overflow-hidden" wire:poll.15s="refreshMessages">
    {{-- Chat Header --}}
    <div class="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-800 text-white">
    ...
    <div
        id="chat-messages"
        class="flex-1 overflow-y-auto px-6 py-4 space-y-4 bg-gray-50 dark:bg-gray-800"
        x-data="{
            scrollToBottom() {
        ...
        @forelse($session->messages as $msg)
            <div
                class="flex {{ $msg->sender_type->value === 'user' ? 'justify-end' : 'justify-start' }}"
                wire:key="message-{{ $msg->id }}">
                <div class="flex items-end space-x-2 max-w-2xl {{ $msg->sender_type->value === 'user' ? 'flex-row-reverse space-x-reverse' : '' }}">
                    {{-- Avatar --}}
                    <div class="flex-shrink-0 w-8 h-8 rounded-full {{ $msg->sender_type->value === 'user' ? 'bg-blue-600' : 'bg-gray-400' }} flex items-center justify-center text-white text-xs font-bold">
                        {{ strtoupper(substr($msg->sender->name ?? 'U', 0, 1)) }}
                    </div>

                    {{-- Message Bubble --}}
                    <div class="flex flex-col {{ $msg->sender_type->value === 'user' ? 'items-end' : 'items-start' }}">
                        <div class="px-4 py-3 rounded-2xl {{ $msg->sender_type->value === 'user'
