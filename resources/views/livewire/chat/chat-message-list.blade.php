        ...
        <div class="flex {{ $message->sender_type->value === 'customer' ? 'justify-end' : 'justify-start' }}">
            <div class="flex gap-2 max-w-[80%] {{ $message->sender_type->value === 'customer' ? 'flex-row-reverse' : 'flex-row' }}">
                ...
                <div class="flex-shrink-0">
                    <div class="w-8 h-8 rounded-full {{ $message->sender_type->value === 'customer' ? 'bg-blue-600' : 'bg-gray-400' }} flex items-center justify-center text-white text-sm font-medium">
                        {{ substr($message->sender?->name ?? 'U', 0, 1) }}
                    </div>
                </div>

                {{-- Message Bubble --}}
                <div class="flex flex-col {{ $message->sender_type->value === 'customer' ? 'items-end' : 'items-start' }}">
                    <div class="px-4 py-2 rounded-lg {{ $message->sender_type->value === 'customer' ? 'bg-blue-600 text-white' : 'bg-white text-gray-900' }} shadow-sm">
